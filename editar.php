...
$MOD2= $_GET['MOD2'];
    ...
    $res = mysqli_query($conexao,"SELECT NOME,Idade,Genero,Turma,Modalidadeindividual,Modalidadeequipe,Altura,Peso,Matricula_aluno,Nota FROM tela_cadastro WHERE ID = $ID");
    ...
    $att = "UPDATE tela_cadastro SET NOME = '$novo_nome', Idade ='$nova_idade', Genero = '$novo_genero', Turma = '$nova_turma', Modalidadeindividual = '$nova_modal', Modalidadeequipe = '$nova_equip', Altura = '$nova_altura', Peso = '$novo_peso', Nota = '$nova_nota' WHERE ID = $ID";

    if ($conexao->query($att) === TRUE) {
        $_SESSION['att_cadastro'] = true;
        header("Location: perfill.php?CODIGO=$dados[8]&MOD1=$MOD1&MOD2=$MOD2");
        exit();
    } else {
    ...
    <ul>
       ...
       <form method="POST" autocomplete="off">
          ...
          <input type="text" name="nome" class="form-control" placeholder="Digite seu Nome Completo" value ="<?php echo $dados[0];?>" required>
                ...
                <div class="col-md-14 mb-3">
        <label for="idade">Nota particular</label>
        <input type="text" name="nota" class="form-control" placeholder="Digite a Nota Desejada " value ="<?php echo $dados[9];?>" required maxlength="3">
           </div>
